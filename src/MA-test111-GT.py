...
class MultiModalTransformer(nn.Module):
    ...
    def forward(self, x_audio, x_visual):
        ...
        combined_input = torch.cat((audio_embedded, visual_embedded), dim=0)

        # Apply transformer encoder
        transformer_output = self.transformer_encoder(combined_input.unsqueeze(1))
...
class Identity(nn.Module):
    """
    Identity module to pass tensor without modification
    """
    def forward(self, x):
        return x
...
# Recreate weighted reference list
...
euclidean_distances_array = [(1 - euclidean_distances(np.array(generated_EPP).reshape(1, -1),
                                                     np.array(real_EPP).reshape(1, -1))[0][0] /
                            (max_generated_EPP - min_generated_EPP)) * 100
                            for generated_EPP, real_EPP in zip_longest(generated_EPP_values, weighted_avg_reference_EPP_list)]
...
# Convert EPP values to binary labels based on a threshold
threshold = 0.2
...
# Sort normalized real and generated EPP values
...
# Create spline interpolation for smoothing
spline_real = UnivariateSpline(range(len(sorted_real_EPP)), sorted_real_EPP, s=0.5)
...
# Linear fitting using least squares
slope, intercept = np.polyfit(x_smooth, y_smooth_generated, 1)
